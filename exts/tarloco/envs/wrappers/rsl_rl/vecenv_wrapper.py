...
.. code-block:: python
    ...
    from exts.tarloco.utils.wrappers.rsl_rl import RslRlVecEnvWrapper
...
class RslRlVecEnvWrapper(VecEnv):
    """Wraps around Isaaclab environment for RSL-RL library
    ...
    def __init__(self, env: ManagerBasedRLEnv, args_cli=None, **kwargs):
        ...
        print(f"Observation manager: {self.unwrapped.observation_manager}")
        ...
        self.env.reset(seed=args_cli.seed)
    ...
    def step_new(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        # record step information
        obs_dict, rew, terminated, truncated, extras = self.env.step(actions)
        # move extra observations to the extras dict
        obs = obs_dict["policy"]
